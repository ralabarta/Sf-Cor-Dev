...
| `hybrid` | Both native-provided locators | Both native-provided locators | Yes |
...
Read only the native-provided locators for both stores; never prefer one, fall back, or substitute a locator.
...
| `hybrid` | Persist through both native-provided state locators | Read only both native-provided state locators; never prefer one, fall back, or substitute a locator |
