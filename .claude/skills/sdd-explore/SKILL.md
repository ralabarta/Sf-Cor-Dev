...
- **openspec**: Read and follow `.claude/skills/_shared/openspec-convention.md` from the project root when present, otherwise `~/.claude/skills/_shared/openspec-convention.md`.
...
Return EXACTLY this format to the orchestrator (and write the same content to the orchestrator-injected exploration artifact locator if saving):
...
### Next Gate
Immediately offer `sdd-research`. If research is not selected, complete and persist the confirmed pre-proposal gate before launching `sdd-propose`.
```

## Rules

- In `openspec` or `hybrid` mode, the ONLY file you MAY create or update is the exploration artifact at the orchestrator-injected locator
