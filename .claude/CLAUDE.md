...
| `rdd_disabled` | RDD is off. Do not enable it from this continuation; ordinary repository policy applies. Only after an explicit user request may gentle-ai review mode enable --scope global be executed, followed by S. |
...
Each launch runs only its own provider-issued `review.capture-result` argument tokens exactly as returned. Completion order is not authority: shared Go admission/election owns reduction and semantics. The final admitted capture owns reduction and closure. On approved, acknowledgement remains pending: follow the returned continuation, re-enter the exact bound STATUS, and execute only the returned review.acknowledge-approved invocation. Authority is burned only after gentle-ai.review-acknowledged/v1; do not FINALIZE. On `correction_required`, continue only through exact bound STATUS and the provider-issued `review.capture-correction-plan` binding. After a malformed or nonterminal capture, reconcile through exact bound STATUS and retry only an identically reoffered slot.
