...
// Returns the full name of the current Sf-Cor-Dev version.
//
// For local dev compiles we try to append the commit SHA and
// commit date from git. If that fails only the local compilation
// date is set and "nogit" is specified:
//      Sf-Cor-Dev dev-YYYYMMDD-SHA
//      or
//      Sf-Cor-Dev dev-YYYYMMDD-nogit
//
// For releases (non-dev builds) we only include the version number:
//      Sf-Cor-Dev version
std::string engine_version_info() {
    std::stringstream ss;
    ss << "Sf-Cor-Dev " << version << std::setfill('0');
...
std::string engine_info(bool to_uci) {
    return engine_version_info() + (to_uci ? "\nid author " : " by ")
         + "the Stockfish and CorChess developers (see AUTHORS file)";
}
